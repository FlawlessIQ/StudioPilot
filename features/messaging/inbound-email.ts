/**
 * Inbound email — the deterministic half.
 *
 * When a client replies to a studio's email, the raw body carries the whole
 * history: their answer on top, then a quote of everything said before, then a
 * signature and a mail-client divider. Storing that verbatim would make a thread
 * unreadable within three exchanges, each message containing all the previous
 * ones.
 *
 * This module extracts what the person actually wrote, and recognises the mail
 * that should never become a message at all — out-of-office notices, bounces,
 * and bulk auto-responders.
 *
 * Pure and browser-safe: no crypto, no I/O. Token signing lives in
 * functions/src/communications/reply-address.ts because it needs a secret.
 * `functions/src/communications/inbound-email.ts` mirrors this; the copy here is
 * the one with unit tests and is the source of truth.
 */
