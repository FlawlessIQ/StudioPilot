/**
 * The workflows a studio starts with — the functions copy.
 *
 * Mirrors features/workflows/starter-templates.ts, which is the source of
 * truth and where the reasoning lives. functions/ is a separate package
 * with no "@/features" path, so the definitions are duplicated here and
 * tests/workflow-starter-templates.test.ts fails if the two drift.
 *
