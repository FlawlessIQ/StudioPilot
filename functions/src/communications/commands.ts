      ...
      if (command.type === "sendMessage") {
        ...
        result = await db.runTransaction(async (transaction) => {
          ...
          if (draft.get("status") !== "approved_unsent") {
            throw new Error("DRAFT_NOT_READY_TO_SEND");
          }
          const recipient = String(draft.get("recipient") ?? "").trim();
          if (!z.string().email().safeParse(recipient).success) {
            throw new Error("DRAFT_RECIPIENT_REQUIRED");
          }
          const emailJobId = `manual_${draft.id}`;
          transaction.update(draftReference, {
            status: "queued",
          ...
          transaction.create(db.doc(`emailJobs/${emailJobId}`), {
            ...
            communicationDraftId: draft.id,
            status:
              draft.get("scheduledFor") && draft.get("scheduledFor") > now
                ? "scheduled"
                : "queued",
            scheduledFor: draft.get("scheduledFor") ?? null,
            attempts: 0,
