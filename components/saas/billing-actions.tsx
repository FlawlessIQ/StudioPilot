...
export function BillingAction({plan,cadence,label}:{plan?:"solo"|"studio"|"multi_brand";cadence?:"monthly"|"yearly";label:string}){
  ...
  const run=async()=>{const endpoint=process.env.NEXT_PUBLIC_BILLING_FUNCTIONS_URL;if(!endpoint){setNotice("Development preview: Stripe Checkout was not opened and no subscription changed.");return}try{const {auth,firestore}=getFirebaseClient();const user=auth.currentUser;if(!user)throw new Error("Sign in to manage billing.");const memberships=await getDocs(query(collection(firestore,"memberships"),where("userId","==",user.uid),where("status","==","active"),limit(1)));const tenantId=memberships.docs[0]?.data().tenantId;if(typeof tenantId!=="string")throw new Error("No active studio membership was found.");const token=await getAppCheckToken();const response=await fetch(`${endpoint.replace(/\/$/,"")}/billingCommand`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${await user.getIdToken()}`,...(token?{"x-firebase-appcheck":token}:{})},body:JSON.stringify({type:plan?"createCheckout":"createPortal",tenantId,plan,cadence})});const result=await response.json() as {url?:string;error?:string};if(!response.ok||!result.url)throw new Error(result.error??"Billing could not be opened.");window.location.assign(result.url)}catch(caught:unknown){setNotice(caught instanceof Error?caught.message:"Billing could not be opened.")}};
