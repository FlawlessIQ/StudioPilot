...
export function PostEventAction({type,input,label,completedLabel}:{type:string;input:Record<string,unknown>;label:string;completedLabel:string}){
  const[interactive,setInteractive]=useState(false);const[done,setDone]=useState(false);const[notice,setNotice]=useState<string|null>(null);
  useEffect(()=>{const frame=requestAnimationFrame(()=>setInteractive(true));return()=>cancelAnimationFrame(frame)},[]);
  const run=async()=>{setNotice(null);try{const response=await sendPostEventCommand(type,input);setDone(true);setNotice(response.persisted?completedLabel:`Development preview: ${completedLabel.toLowerCase()} No server record was changed.`)}catch(caught:unknown){setNotice(caught instanceof Error?caught.message:"The action could not be completed.")}};
