...
export function CrewPortalShell({ children, active }: { children: React.ReactNode; active?: string }) {
  const shellMounted = useContext(CrewShellContext);
  if (shellMounted) return <>{children}</>;
  return <CrewShellContext.Provider value><WorkspaceProvider area="crew"><AuthBoundary area="crew"><CrewShell active={active}>{children}</CrewShell></AuthBoundary></WorkspaceProvider></CrewShellContext.Provider>;
}
...
function CrewShell({ children, active }: { children: React.ReactNode; active?: string }) {
  ...
  return <div className={navigationOpen ? "crew-portal-frame crew-navigation-open" : "crew-portal-frame"}>
    ...
    <main className="crew-portal-content"><header><button aria-controls="crew-navigation" aria-expanded={navigationOpen} className="mobile-menu" onClick={() => setNavigationOpen(true)} type="button" aria-label="Open crew navigation"><Menu size={20}/></button><span className="crew-page-context">{resolvedActive}</span><span className="crew-studio-name">{workspace.tenantName}</span><SignOutButton/></header>{children}</main>
  </div>;
}
