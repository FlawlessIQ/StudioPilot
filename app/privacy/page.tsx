  ...
  return (
    <main className="ds-root legal-page" data-ds-theme="emerald">
      ...
      <article>
        ...
        <p>If a studio connects Google Calendar, StudioCue processes the OAuth authorization details for that Google Account and the calendar information needed to run the studio&apos;s scheduling workflow. With read access it retrieves the studio&apos;s list of calendars, so the owner can choose which calendar governs availability, and free/busy intervals for that calendar, so clients are only offered consultation times the studio is actually free. With write access it creates the consultation and event-day entries for that studio&apos;s booked work. StudioCue acts only on entries it created, identified by the event identifier stored with the corresponding consultation or project. It does not list or read the contents of a studio&apos;s other calendar entries. It does not currently modify or remove an entry after creating it, and removing an entry in Google Calendar does not change the corresponding record in StudioCue.</p>
