  ...
  return (
    <main className="ds-root legal-page" data-ds-theme="emerald">
      ...
      <article>
        ...
        <p>If a studio connects Google Calendar, StudioCue processes the OAuth authorization details for that Google Account and the calendar information needed to run the studio&apos;s scheduling workflow. With read access it reads only free/busy intervals for the Google Account&apos;s primary calendar, so clients are only offered consultation times the studio is actually free; the same query over a one-minute window is how StudioCue checks that the connection is still working. With write access it creates the consultation and event-day entries for that studio&apos;s booked work on that same calendar, and updates or removes a consultation entry when the studio reschedules or cancels it. StudioCue acts only on entries it created, identified by the event identifier stored with the corresponding consultation or project. The scopes StudioCue requests do not permit listing a studio&apos;s calendars or reading the contents of any calendar entry. Removing an entry directly in Google Calendar does not change the corresponding record in StudioCue.</p>
