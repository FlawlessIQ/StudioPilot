...
| Stripe Connect | Webhook verified; per-studio OAuth pending one value | The live Connect invoice webhook and signing secret are configured and deployed. The per-studio Connect OAuth path is complete in code and needs only `STRIPE_CLIENT_ID` (the `ca_…` from Stripe Dashboard → Connect → Settings): `config()` reuses `STRIPE_SECRET_KEY` as the token-exchange `client_secret` because Stripe issues no separate OAuth secret, the exchange correctly omits `redirect_uri` for Stripe, and the callback reads `stripe_user_id` as the account id. Register `https://studio-cue.com/api/integrations/oauth/callback` in Stripe's Connect redirect URIs at the same time — Stripe validates the redirect against the Connect app's own settings rather than the token request, so a missing entry fails the same way Intuit's and Zoom's did. Stripe serves the `invoicing` capability, so it is a genuine alternative to QuickBooks rather than an addition. |
| QuickBooks Online | Re-gated August 19, 2026; blocked inside Intuit's app configuration | Removed from `NEXT_PUBLIC_ENABLED_OAUTH_PROVIDERS` again, because the Connect button provably could not finish and this file's own rule is to enable a provider only once its evidence passes. The card states the real reason. Everything on the StudioCue side is verified — see the investigation record below. |
...
callback is registered. Worse for Intuit, a successful connect proves it only
for the keypair that was in use: Development and Production keys carry separate
Redirect URIs lists, so swapping keys can re-open a redirect question that a
previous connect appeared to have settled. Each was tested with a deliberately unregistered
